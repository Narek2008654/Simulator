...
def build_q_model(state_shape, n_actions = 9, hidden_units=(5, 5), learning_rate=1e-3):
    """
    Build a simple fully-connected Q-network.
    - state_shape: tuple, shape of a single state (e.g. (3,), (3,1,1) or flattened)
    - n_actions: int, number of discrete actions
    Returns compiled Keras model.
    """
    # normalize shape to 1-D vector for Dense layers
    ...
    # Q-values (one per action) -> linear output (no softmax)
...
if __name__ == "__main__":
    ...
    # Save
    save_model(q_model, "./model_savings/dqn_from_dataset.keras")
